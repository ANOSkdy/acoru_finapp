    ...
    const result = await pool.query(
      `WITH entries AS (
         ...
         SELECT
           ...
           e.account_type,
           CASE
             WHEN e.account_type = 'revenue' THEN CASE WHEN e.side = 'credit' THEN e.amount ELSE -e.amount END
             WHEN e.account_type = 'expense' THEN CASE WHEN e.side = 'debit' THEN e.amount ELSE -e.amount END
             ELSE 0
       ...
       SELECT
         mapped.line_code,
         COALESCE(mapped.line_name, CASE WHEN mapped.account_type = 'revenue' THEN '売上' ELSE '費用' END) AS line_name,
         mapped.account_code,
         mapped.account_name,
       ...
       GROUP BY
         mapped.line_code,
         COALESCE(mapped.line_name, CASE WHEN mapped.account_type = 'revenue' THEN '売上' ELSE '費用' END),
         mapped.account_code,
         mapped.account_name,
         mapped.statement_type,
         mapped.mapping_status,
       ...
       ORDER BY
         CASE mapped.account_type WHEN 'revenue' THEN 0 WHEN 'expense' THEN 1 ELSE 2 END,
         COALESCE(mapped.display_order, 999999),
