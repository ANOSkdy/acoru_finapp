...
export default function DepartmentsPage() {
  ...
  async function load() {
    ...
    const json = (await res.json()) as { ok?: boolean; rows?: Department[]; error?: { message?: string } };
    ...
    <section style={{ display: "grid", gap: 12 }}>
      ...
      <div style={{ overflowX: "auto" }}>
        <table className="record-table" style={{ minWidth: 680 }}>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.department_id}><td>{r.code}</td><td>{r.name}</td><td>{r.is_active ? "有効" : "無効"}</td><td>{String(r.created_at).slice(0, 19).replace("T", " ")}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
